package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private const const_2144:int = 13421772;
      
      private const const_2143:int = 16777215;
      
      private const const_374:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1822 = Math.max(this.var_1822,_loc6_.colorLayerIndex);
            }
         }
         this.var_435 = this.var_66.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_707 != null && !this.var_1824)
            {
               _loc3_ = this.var_707;
            }
            else
            ...
            if(this.var_777)
            {
               switch(this.var_777.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_47:
                     _loc2_.style = HabboIconType.const_885;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1462.color = this.const_2144;
         }
         else
         {
            this.var_1462.color = this.const_2143;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_374[_loc6_] + "_" + FigureData.const_153;
               _loc4_ = this.var_435.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_374.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_374[_loc6_] + "_" + FigureData.const_153;
                  _loc4_ = this.var_435.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_626 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1823)
               {
                  ...
                  this.var_1823 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_626.width,this.var_626.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_626.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_626.y;
               if(this.var_1824 && _loc2_.colorLayerIndex > 0)
               {
