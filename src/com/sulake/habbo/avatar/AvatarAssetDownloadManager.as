package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_958:int = 3;
      
      private var var_434:AssetLoaderStruct;
      
      private var var_956:Timer;
      ...
      private const const_2154:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_956 = new Timer(this.const_2154,1);
         this.var_956.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_434)
         {
            this.var_434.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_434)
         {
            this.var_434.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_958);
            if(this.var_1461.indexOf("?") > 0)
            {
               _loc2_ = this.var_1461 + "&retry=" + this.var_958;
            }
            else
            {
               _loc2_ = this.var_1461 + "?retry=" + this.var_958;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2473,this._assets);
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1821)
         {
            this.var_1188.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_957)
         {
            _loc7_ = true;
            _loc4_ = this.var_957[_loc3_];
      ...
      private function getLibsToDownload(param1:IAvatarFigureContainer) : Array
      {
         ...
         var _loc4_:Array = param1.method_5();
         for each(_loc5_ in _loc4_)
         {
      ...
      {
         var _loc1_:* = null;
         while(this.var_775.length > 0 && this.var_776.length < this.const_1850)
         {
            _loc1_ = this.var_775[0];
            _loc1_.startDownloading();
            this.var_776.push(this.var_775.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_775.indexOf(param1) == -1 && this.var_776.indexOf(param1) == -1)
         {
