package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_549:IKeyExchange;
      
      private var var_1016:String;
      
      private var _view:HabboLoginDemoView;
      
      private var var_2717:PseudoRandom;
      ...
      public function setSSOTicket(param1:String) : void
      {
         ...
         {
            this.var_663 = param1;
            this.var_55.initConnection(HabboConnectionType.const_291);
         }
      }
      ...
      {
         if(this.var_55 && this.var_135 && this._roomSessionManager && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         var _loc1_:Boolean = this.var_135.getBoolean("client.hotel_view.show_on_startup",true);
         this.var_235 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         var _loc2_:String = this.var_135.getKey("client.hotel_view.image");
         var _loc3_:String = this.var_135.getKey("image.library.url");
         if(_loc2_ != null && _loc3_ != null)
         {
            this.var_235.loadHotelViewImage(_loc3_ + _loc2_);
         }
         var _loc4_:* = this.var_135.getKey("use.sso.ticket") == "1";
         this.var_663 = this.var_135.getKey("sso.ticket",null);
         this.var_1983 = this.var_135.getKey("flash.client.url","");
         this.var_2719 = this.var_135.getKey("external.variables.txt","");
         if(this.var_235 == null)
         {
            this.var_235 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         }
         if(_loc4_)
         {
            this.var_55.mode = HabboConnectionType.const_92;
            if(this.var_663)
            {
               this.var_55.initConnection(HabboConnectionType.const_291);
            }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc5_)
            {
               ...
               this._view.addEventListener(HabboLoginDemoView.const_550,this.onInitConnection);
            }
            else
            {
               Core.crash("Login without an SSO ticket is not supported",Core.const_1598);
            }
         }
      }
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         {
            this.var_55.mode = HabboConnectionType.const_92;
            if(this.var_135.keyExists("local.environment"))
            {
               this.var_55.mode = HabboConnectionType.const_1256;
            }
         }
         this.var_55.initConnection(HabboConnectionType.const_291);
      }
      ...
      private function onConnectionEstablished(param1:Event = null) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_827);
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2717 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_55.mode)
         {
            case HabboConnectionType.const_92:
               _loc6_ = "";
               _loc6_ = this.var_135.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_135.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1531 = _loc4_;
               this.var_235.loadBannerImage(_loc7_ + "?token=" + this.var_1531,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1256:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1797:
               this.var_1016 = this.generateRandomHexString(30);
               this.var_549 = new DiffieHellman(new BigInteger(this.var_135.getKey("connection.development.prime"),16),new BigInteger(this.var_135.getKey("connection.development.generator"),16));
               this.var_549.init(this.var_1016);
               _loc8_ = this.var_549.getPublicKey(10);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         this.var_549.generateSharedKey(_loc4_,10);
         var _loc5_:String = this.var_549.getSharedKey(16).toUpperCase();
         var _loc6_:RC4_R27 = new RC4_R27(null,null);
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2717);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         connection.send(new VersionCheckMessageComposer(401,this.var_1983,this.var_2719));
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(param1.type == Event.CLOSE && !this.var_2718)
         {
            _loc2_ = this.var_135.getKey("logout.disconnect.url");
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2718 = true;
         var _loc2_:String = this.var_135.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_135.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1531);
         ...
         this.var_549 = new DiffieHellman(_loc10_,_loc11_);
         var _loc13_:int = 10;
         var _loc14_:* = null;
         while(_loc13_ > 0)
         {
            _loc14_ = this.generateRandomHexString(30);
            this.var_549.init(_loc14_);
            _loc15_ = this.var_549.getPublicKey(10);
