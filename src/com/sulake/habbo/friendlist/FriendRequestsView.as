package com.sulake.habbo.friendlist
{
   ...
   public class FriendRequestsView implements ITabView, IFriendRequestsView
   {
      ...
      private static const const_1908:String = "no_reqs_info";
      ...
      private var var_2026:IContainerButtonWindow;
      ...
      {
         this.var_2026 = IContainerButtonWindow(param1.findChildByName("accept_all_but"));
         this.var_2025 = IContainerButtonWindow(param1.findChildByName("reject_all_but"));
         this.var_2025.procedure = this.onDismissAllButtonClick;
         this.var_2026.procedure = this.onAcceptAllButtonClick;
         this.refreshButtons();
      }
      ...
      public function declineRequest(param1:int) : void
      {
         ...
         if(this._friendList && this._friendList.events)
         {
            this._friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_333,param1));
         }
      }
      ...
      private function onDismissAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            if(_loc4_.state != FriendRequest.const_409 && _loc4_.state != FriendRequest.const_437)
            {
               _loc4_.state = FriendRequest.const_437;
               this.refreshRequestEntry(_loc4_);
               if(this._friendList && this._friendList.events)
               {
                  this._friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_333,_loc4_.requestId));
               }
            }
         }
         this.refresh();
      ...
      private function onAcceptAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         for each(_loc4_ in this._friendList.friendRequests.requests)
         {
            if(_loc4_.state != FriendRequest.const_409 && _loc4_.state != FriendRequest.const_437)
            {
               ...
               _loc4_.state = FriendRequest.const_409;
               this.refreshRequestEntry(_loc4_);
               if(this._friendList && this._friendList.events)
               {
      ...
      private function refreshButtons() : void
      {
         ...
         this.setEnabled(this.var_2026,_loc1_);
