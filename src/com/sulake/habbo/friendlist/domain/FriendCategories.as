package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_406);
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               if(currentlyOnline && !updatedFriend.online)
               {
                  ...
                  this.var_470.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_470.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_470.view.setNewMessageArrived();
                  this.var_470.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_406).received = true;
         this.findCategory(FriendCategory.const_1307).received = true;
